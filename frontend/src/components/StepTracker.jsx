...
function StepBody({ step, readOnly, onUpdateStep }) {
  ...
  const hasContent = step.notes || step.resultado_porcentaje != null || (step.updated_by_name && step.status !== "pending") || (!readOnly && onUpdateStep && transitions.length > 0);
  if (!hasContent) return null;
  return (
    <div className="px-3.5 pb-2.5 space-y-2 border-t border-gray-100 dark:border-gray-700/60 pt-2.5">
      {step.resultado_porcentaje != null && (
        <p className="text-[11px] text-green-600 dark:text-green-400 font-medium bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-lg px-2.5 py-1.5">
          Resultado: {step.resultado_porcentaje}%
        </p>
      )}
      {step.notes && (
