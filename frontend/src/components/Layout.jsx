...
import { FlaskConical, LayoutDashboard, Settings, LogOut, Sun, Moon, User, ChevronRight, BarChart2, Menu, History } from "lucide-react";
...
function getPageTitle(pathname, analysts) {
  ...
  if (pathname === "/confirmaciones-jt") return "Confirmaciones JT";
...
function LayoutInner({ children }) {
  ...
  const { navActions, navCenter } = useNavActions();
  const location = useLocation();
  const navigate = useNavigate();
  const [analysts, setAnalysts] = useState([]);
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [pendingBossCount, setPendingBossCount] = useState(0);

  // Cierra sidebar al navegar en móvil
  useEffect(() => { setSidebarOpen(false); }, [location.pathname]);

  useEffect(() => {
    if (!["admin", "analyst"].includes(user?.role)) { setAnalysts([]); return; }
    axios.get("/api/users/analysts").then(r => setAnalysts(r.data)).catch(() => setAnalysts([]));
  }, [user?.role]);

  useEffect(() => {
    if (user?.role !== "jefe_turno") { setPendingBossCount(0); return; }
    const load = () =>
      axios.get("/api/samples/pending-approval")
        .then(r => setPendingBossCount(Array.isArray(r.data) ? r.data.length : 0))
        .catch(() => setPendingBossCount(0));
    load();
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-gray-950 overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 lg:w-60 lg:relative lg:z-auto flex-shrink-0 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col shadow-sm dark:shadow-none transform transition-transform duration-200 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        ...
        <nav className="flex-1 p-2 space-y-1 overflow-y-auto scrollbar-thin">
          ...
          >
            ...
            <span className="flex-1 hidden lg:block text-sm font-medium">Dashboard</span>
            {user?.role === "jefe_turno" && pendingBossCount > 0 && (
              <span
                className={`min-w-[1.25rem] h-5 px-1 rounded-full text-[10px] font-bold flex items-center justify-center
                  ${location.pathname === "/dashboard" ? "bg-white text-indigo-600" : "bg-amber-500 text-white"}`}
                title="Muestras pendientes de confirmación del turno">
                {pendingBossCount > 9 ? "9+" : pendingBossCount}
              </span>
            )}
          </Link>
          ...
          {user?.role === "jefe_turno" && (
            <Link to="/confirmaciones-jt"
              className={`flex items-center gap-2 px-3 py-2.5 rounded-lg transition-all mt-2
                ${location.pathname === "/confirmaciones-jt"
                  ? "bg-indigo-600 text-white"
                  : "text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white"}`}
            >
              <History size={18} className="flex-shrink-0" />
              <span className="flex-1 hidden lg:block text-sm font-medium">Confirmaciones JT</span>
            </Link>
          )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top navbar */}
        <header className="h-14 flex-shrink-0 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex items-center gap-2 px-3 sm:px-5 shadow-sm dark:shadow-none">
          {/* Hamburger (mobile only) */}
          <button onClick={() => setSidebarOpen(v => !v)}
            className="lg:hidden p-2 rounded-lg text-gray-400 hover:text-gray-700 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800 transition-all flex-shrink-0">
            <Menu size={18} />
          </button>

          {/* Page title / breadcrumb */}
          <div className="flex items-center gap-2 min-w-0 shrink max-w-[38%] sm:max-w-[45%]">
            {isAnalystPage && (
              <>
          ...
          {/* Centro: búsqueda inyectada por página (Dashboard, Admin, etc.) */}
