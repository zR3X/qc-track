...
function UsersTab({ listSearch = "" }) {
  ...
  const ROLE_COLORS = {
    ...
    jefe_turno: "text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950 border-amber-300 dark:border-amber-800"
  };
...
export default function AdminPanel() {
  ...
  const { setNavCenter } = useNavActions();

  const handleFilterChange = useCallback((filters) => {
    setListSearch(filters.search || "");
  }, []);
  ...
  useEffect(() => {
    setNavCenter(
      <SearchBar
        placeholder="Buscar usuario, rol o paso QC..."
        onFilterChange={handleFilterChange}
        showStatusFilter={false}
        showPlantaFilter={false}
  ...
  return (
    <div className="p-4 lg:p-5">
      ...
      <div className="animate-fade-in max-w-2xl">
        {tab === "users" ? <UsersTab listSearch={listSearch} /> : <StepsTab listSearch={listSearch} />}
      </div>
    </div>
  );
}
